...
import { createVideo } from '@/actions/media.action';
...
export default function CreateVideoForm() {
  ...
  const form = useForm<TCreateVideo>({
    resolver: zodResolver(ZCreateVideo) as any,
    defaultValues: {
      title: '',
      titleAm: null,
      titleOr: null,
      ...
      <Form {...form}>
        <form
          onSubmit={form.handleSubmit((data) => {
            mutate(async () => await createVideo(data), {
              onSuccess: (created) => {
                if (created && 'id' in created) {
                  router.push(`/admin/media/videos/${created.id}`);
                }
              },
